...
while True:
    ...
    faceDetection = faceTrain.detectMultiScale(gs, scaleFactor=1.1, minNeighbors=5,minSize=(60,60),flags=cv2.CASCADE_SCALE_IMAGE)
    for(x,y,w,h) in faceDetection:
        cv2.putText(frame,"Face",(x,y-50),cv2.FONT_ITALIC, 2,(255,255,255),3,cv2.LINE_AA)
        ...
        for (x2, y2, w2, h2) in eyes:
            ...
            cv2.putText(frame,"Eye",(x+x2,y+y2-50),cv2.FONT_ITALIC, 1,(255,255,0),3,cv2.LINE_AA)
            frame = cv2.circle(frame, eye_center, radius, (255,255,0), 4)

        for(x3,y3,w3,h3) in mouth:
            cv2.putText(frame,"Smile :)",(x+x3,y+y3-50),cv2.FONT_ITALIC, 1,(0,255,0),3,cv2.LINE_AA)
            frame = cv2.rectangle(frame,(x+x3,y+y3),(x+x3+w3,y+y3+h3),(0,255,0),1)
    ...
    if time.time()-oldTime>10:
        ...
        if(len(faceDetection)==0):
            print("WARNING: No face found. Nothing has been saved to JSON or image logs.")
        else:
            imageGrabber.takePicture(frame)
            soundThreader.playSound("siren","mp3")
            jsonLogger.replaceLogs(faceDetection)
            print("Face has been found. Saved into image logs and logging position and last seen date into JSON.")

    if cv2.waitKey(1) & 0xFF is ord('e'):
        print("Recording has been terminated. Thanks for experimenting with the algorithm!")
        break
